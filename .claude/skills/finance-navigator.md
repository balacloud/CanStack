...
7. How different investment income is taxed outside a registered account, and why that determines what belongs in a TFSA — the comparison deliberately left out of the TFSA series' REIT post, worked out in full during a 2026-09-22 Q&A:
   ...
   - **Second school of thought — growth-maximization, arguably more mainstream (added 2026-09-22, verified via GetSmarterAboutMoney.ca)**: the tax-efficiency ranking above isn't the only lens, and presenting it alone is incomplete. GetSmarterAboutMoney.ca (OSC) explicitly frames a TFSA as "particularly valuable for long-term growth investments" — the logic being there's no cap on how much the account can *grow* to, only on contributions, so a small contribution that grows large returns a much bigger absolute tax-free dollar amount than optimizing which income type is sheltered. In practice, broad growth/equity ETFs (including all-in-ones like XEQT/VEQT) are probably the single most commonly recommended default TFSA holding in the Canadian personal-finance community — more about simplicity and time horizon than micro-optimizing by asset type. Present BOTH schools of thought when this topic is drafted (shelter-what's-taxed-worst vs. shelter-what-will-grow-most) rather than picking one as "the" answer — this is a genuinely debated topic, not a settled fact, and framing it as two legitimate views matches the neutral/evidence-based content bar (guardrail #1).
   - **GICs/bonds note**: technically the same "full marginal rate outside" mechanism as REITs (interest gets no tax discount outside either), but community consensus — and this content's own Day 1 post — argues against treating them as a "great TFSA fit" in practice, since typical yields are low enough that the absolute dollar tax saved is trivial. Don't contradict Day 1's "don't waste TFSA room on cash/GICs" guidance when this topic is drafted.
